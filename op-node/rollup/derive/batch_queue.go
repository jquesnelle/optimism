...
// AddL1Block adds an Origin to the BatchQueue.
func (bq *BatchQueue) AddL1Block(l1Block eth.L1BlockRef) error {
...
func (bq *BatchQueue) followingBatches(l2SafeHead *eth.L2BlockRef) []*BatchWithOrigin {
	...
	for {
		...
		for _, batch := range batches {
			...
			if !ValidBatch(batch.Batch, bq.config, batch.Batch.Epoch, minL2Time, maxL2Time) {
				break
			}
