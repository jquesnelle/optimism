...
task('genesis', 'create a genesis config')
  .addOptionalParam(
    'outfile',
  ...
  .setAction(async (args, hre) => {
    ...
    // Deploying to an already existing network, the existing address
    // should be used
    let ProxyL1CrossDomainMessenger = await hre.deployments.getOrNull(
      'Proxy__OVM_L1CrossDomainMessenger'
    )
    // When deploying to a fresh network, use the freshly deployed
    // contract. TODO: this will be placed behind a proxy eventually
    ...
    const variables = {
      ...
      L2CrossDomainMessenger: {
        _initialized: 1,
        _owner: hre.deployConfig.l2CrossDomainMessengerOwner,
      ...
      GasPriceOracle: {
        _owner: hre.deployConfig.gasPriceOracleOwner,
      ...
      L1Block: {
        number: hre.deployConfig.l1BlockInitialNumber,
        timestamp: hre.deployConfig.l1BlockInitialTimestamp,
        basefee: hre.deployConfig.l1BlockInitialBasefee,
        hash: hre.deployConfig.l1BlockInitialHash,
        sequenceNumber: hre.deployConfig.l1BlockInitialSequenceNumber,
      },
      OVM_ETH: {
      ...
      GovernanceToken: {
        name: 'Optimism',
        symbol: 'OP',
        _owner: hre.deployConfig.proxyAdmin,
      },
    }

    assertEvenLength(implementationSlot)
    assertEvenLength(adminSlot)
    assertEvenLength(hre.deployConfig.proxyAdmin)

    const predeployAddrs = new Set(Object.values(predeploys))
    ...
    for (let i = 0; i <= 0xffff; i++) {
      ...
      alloc[addr] = {
        ...
        storage: {
          [adminSlot]: hre.deployConfig.proxyAdmin,
        },
      }
    ...
    if (hre.deployConfig.fundDevAccounts) {
      const accounts = [
    ...
    const genesis: Genesis | any = {
      config: {
        chainId: hre.deployConfig.genesisBlockChainid,
        homesteadBlock: 0,
        eip150Block: 0,
        eip155Block: 0,
        eip158Block: 0,
      ...
      gasLimit: hre.deployConfig.genesisBlockGasLimit,
      extraData: hre.deployConfig.genesisBlockExtradata,
      alloc,
    }
